...
public final class IntoMappingAction<T> {
    ...
    @NotNull
    public <S> IntoMappingAction<T> from(@NotNull Type<S> source, @NotNull Mapper<S, T> mapper) {
        return from(source, mapper, proteus.conflictStrategy());
    }
    ...
    public <S> IntoMappingAction<T> from(@NotNull Type<S> source, @NotNull Mapper<S, T> mapper, @NotNull ProteusBuilder.ConflictStrategy strategy) {
        sources.forEach(target -> proteus.register(source, (Type<T>) target, mapper, strategy));
        return this;
    }
}
